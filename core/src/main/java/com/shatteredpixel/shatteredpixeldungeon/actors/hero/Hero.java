...
public class Hero extends Char {
	...
	public float attackDelay() {
		...
		if (!RingOfForce.fightingUnarmed(this)) {
			...
			result = delay * belongings.attackingWeapon().delayFactor( this );
		...
		Rabies rab = buff(Rabies.class);
		if (rab != null && rab.stage >= 2){
			result *= 0.8f;
		}
	...
	public boolean act() {
		...
		if (curAction == null) {
			...
			if(Dungeon.level.feeling == Level.Feeling.DOOM && Random.Float() > 0.99){
				Dungeon.level.triggerDoom();
			}
			resting = false;
	...
	private void forgetItem(){
		if(Math.random() > 0.5) { // 50/50 chance
			...
			if (!known.isEmpty()) {
				...
				GLog.n(Messages.get(this, "forget_scroll"));
			}
		}
		else{
			...
			if (!known.isEmpty()) {
				...
				chosen.setKnown(false);
				Item.updateQuickslot();
				GLog.n(Messages.get(this, "forget_potion"));
			}
		}
	...
	public void die( Object cause ) {
		...
		for (Ankh i : belongings.getAllItems(Ankh.class)){
			if ((ankh == null || i.isBlessed()) && i.isActivated()) {
				ankh = i;
			}
		}
